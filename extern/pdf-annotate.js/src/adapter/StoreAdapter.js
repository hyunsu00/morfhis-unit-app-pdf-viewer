...
export default class StoreAdapter {
   ...
   */
  __addComment(documentId, annotationId, content, dataString, author) { abstractFunction('addComment'); }
  get addComment() { return this.__addComment; }
  set addComment(fn) {
    this.__addComment = function addComment(documentId, annotationId, content, dataString, author) {
      return fn(...arguments).then((comment) => {
        fireEvent('comment:add', documentId, annotationId, comment, dataString, author);
        return comment;
      });
    };
  }
