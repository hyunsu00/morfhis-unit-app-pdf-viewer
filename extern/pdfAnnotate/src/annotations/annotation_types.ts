...
export class BaseAnnotationObj implements BaseAnnotation {
    ...
    public writeAnnotationObject(cryptoInterface : CryptoInterface) : number[] {
        ...
        if (this.border) {
            if (this.border.border_style && this.border.border_style.length > 1) {
                ret.push(WriterUtil.SPACE)
                ...
                let array: number[] = this.border.border_style
                ...
                ret = ret.concat(WriterUtil.BS_S)
                ret.push(WriterUtil.SPACE)
                ret = ret.concat(WriterUtil.AP_D)
                ret.push(WriterUtil.SPACE)

                ret = ret.concat(WriterUtil.BS_W)
                ret.push(WriterUtil.SPACE)
                ret = ret.concat(Util.convertNumberToCharArray((this.border.border_width === undefined) ? 1 : this.border.border_width))
                ret.push(WriterUtil.SPACE)
                ...
                ret.push(WriterUtil.SPACE)
                ret = ret.concat(WriterUtil.writeNumberArray([(this.border.horizontal_corner_radius === undefined) ? 0 : this.border.horizontal_corner_radius,
                    (this.border.vertical_corner_radius === undefined) ? 0 : this.border.vertical_corner_radius,
                    (this.border.border_width === undefined) ? 1 : this.border.border_width]))
                ret.push(WriterUtil.SPACE)
            }
        }
...
export interface MarkupAnnotation extends BaseAnnotation {
    ...
    richtextStringContent?: string // /RC
...
export class MarkupAnnotationObj extends BaseAnnotationObj implements MarkupAnnotation {
    ...
    public writeAnnotationObject(cryptoInterface : CryptoInterface) : number[] {
        ...
        if (this.author != null) {
            ...
            ret = ret.concat(Array.from(Util.escapeString(cryptoInterface.encrypt(Util.convertStringToUnicodeOrAscii(this.author), this.object_id))))
            ret.push(WriterUtil.BRACKET_END)
            ret.push(WriterUtil.SPACE)
        }
        ...
        if (this.richtextString && this.richtextStringContent) {
            ret = ret.concat(WriterUtil.RC)
            ret.push(WriterUtil.SPACE)
            ret.push(WriterUtil.BRACKET_START)
            ret = ret.concat(Array.from(Util.escapeString(cryptoInterface.encrypt(this.startRichTextAsHtml(this.richtextString), this.object_id))))
            ret = ret.concat(Array.from(Util.escapeString(cryptoInterface.encrypt(new Uint8Array(Util.convertStringToHexString(this.richtextStringContent)), this.object_id)))) 
            ret = ret.concat(Array.from(Util.escapeString(cryptoInterface.encrypt(this.endRichTextAsHtml(), this.object_id))))
            ret.push(WriterUtil.BRACKET_END)
            ret.push(WriterUtil.SPACE)
        }
    ...
    public startRichTextAsHtml(richTextResult : any) : Uint8Array {
       return this.getStartRichTextAsHtml(
            richTextResult.fontColor, richTextResult.fontFamily, richTextResult.fontSize, 
            richTextResult.isBold, richTextResult.isItalic, 
            richTextResult.isUnderline, richTextResult.isLineThrough);
    }

    public getStartRichTextAsHtml(fontColor : string = '#FF0000', fontFamily : string = "/Helvetica", fontSize : number = 12, isBold : boolean = false, isItalic : boolean = false, isUnderline : boolean = false, isLineThrough : boolean = false) : Uint8Array {
        let boldStr = isBold ? 'bold': 'normal';
		let italicStr = isItalic ? 'italic': 'normal';
		
		let textDecorationStr = 'none';
		
		if (isUnderline && isLineThrough) {
			textDecorationStr = 'word line-through';
		} else {
			if(isUnderline) {
				textDecorationStr = 'word';
        ...
        output += "<body xmlns=\"http://www.w3.org/1999/xhtml\" xmlns:xfa=\"http://www.xfa.org/schema/xfa-data/1.0/\" xfa:APIVersion=\"Acrobat:10.1.3\" xfa:spec=\"2.0.2\">";
		output += "<p>";
		output += "<span style=\"";
		output += `font-family:${fontFamily};`;
		output += `font-size:${fontSize}pt;`;
		output += `font-weight:${boldStr};`;
		output += `font-style:${italicStr};`;
		output += `color:${fontColor};`;
		output += `text-decoration:${textDecorationStr}\">`;

        return new Uint8Array(Util.convertStringToAscii(output));
    }

    public endRichTextAsHtml() : Uint8Array {
        let output: string = ""
		output += "</span>";
		output += "</p>";
		output += "</body>";
