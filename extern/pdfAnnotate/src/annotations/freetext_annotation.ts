...
import { AppStream, XObjectObj, GraphicsStateParameter, ImageObj } from '../appearance-stream';
...
export class FreeTextAnnotationExObj extends FreeTextAnnotationObj {
    point: number[] = []
    imageUrl: string = ""
    ...
    public createDefaultAppearanceStream() {
        ...
        let imageData = Util.creagtePngImageData(this.imageUrl);
        let pngObj = Util.createPngImageStream(imageData);
        if (!pngObj) {
            return;
        }
        ...
        xobj.object_id = this.factory.parser.getFreeObjectId()
        xobj.new_object = true
        xobj.bBox = [0, 0, pngObj.width, pngObj.height]
        ...
        let cs  = new ContentStream()
        xobj.contentStream = cs
        let go = cs.addGraphicObject()
        let go2 = go.addGraphicObject()
        go2.addOperator("cm", [pngObj.width, 0, 0, pngObj.height, 0, 0])
        go2.addOperator("Do", ['/Im0'])

        // /ProcSet
        xobj.resources.addProcSetDef({name : '/PDF'})
        xobj.resources.addProcSetDef({name : '/Text'})
        xobj.resources.addProcSetDef({name : '/ImageB'})
        xobj.resources.addProcSetDef({name : '/ImageC'})
        xobj.resources.addProcSetDef({name : '/ImageI'})
    
        // mask image
        let maskImageObj = new ImageObj(
            this.factory.parser.getFreeObjectId(),
            pngObj.alphaData,
            pngObj.bitsPerComponent,
            'DeviceGray',
            pngObj.width,
            pngObj.height
        );
        this.additional_objects_to_write.push({obj: maskImageObj, func: ((ob: any, cryptoInterface : CryptoInterface) => ob.writeImage(cryptoInterface))})
        ...
        let imageObj = new ImageObj(
            ...
            pngObj.width,
            pngObj.height,
            maskImageObj.object_id
        )
        xobj.resources.addXObjectDef({name: '/Im0', refPtr: imageObj.object_id})
        this.additional_objects_to_write.push({obj: imageObj, func: ((ob: any, cryptoInterface : CryptoInterface) => ob.writeImage(cryptoInterface))})
