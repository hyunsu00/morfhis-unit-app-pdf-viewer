...
import { FreeTextAnnotationObj, FreeTextAnnotationExObj } from './annotations/freetext_annotation';
...
export class AnnotationFactory {
    ...
    private generateUniqueIdentifier() {
        return "HNC-" + Util.convertDateToPDFDate(new Date()).slice(4, 14) + "-" + this.annotations.length
    }
    ...
    createFreeTextAnnotationEx(...values : any[]) : FreeTextAnnotationExObj {
        let params = ParameterParser.parseParameters(values)

        let annot : FreeTextAnnotationExObj = new FreeTextAnnotationExObj()
        annot = (<any>Object).assign(annot, this.createBaseAnnotation(params.page))
        annot = (<any>Object).assign(annot, params)
