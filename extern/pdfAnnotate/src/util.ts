...
export class Util {
    ...
    public static createPngImage(
        ...
        fontSize : number = 18, 
        opacity : number | undefined) : Uint8Array {
        
        let result : Uint8Array = new Uint8Array();
        
        let canvas : HTMLCanvasElement = <HTMLCanvasElement>document.createElement("canvas");
        const scaleX : number = 2, scaleY = 2;
        let w =  imageSize.cx, h = imageSize.cy;
        ...
        if (textColor) {
            ctx.fillStyle = `rgba(${textColor.r}, ${textColor.g}, ${textColor.b}, ${opacity ?? 1})`;
        }
        ctx.fillText(contents, x, y);
        let imgString = canvas.toDataURL("image/png");
        result = Util.creagtePngImageData(imgString);

        return result;
    }
    
    public static creagtePngImageData(imgString : string) {
        imgString = unescape(imgString);
        imgString = Util.convertBase64ToBinaryString(imgString);

        let result = Util.convertBinaryStringToUint8Array(imgString);
        return result;
    }

    public static createPngImageStream(imageData : Uint8Array) {
    ...
    private static convertBase64ToBinaryString(value : string) : string {
        if (value == undefined || value == null) { throw "Property or Arguments was Never Null."; }
        else {
            if (typeof (value) != "string") { throw "Property or Arguments was not 'String' Types."; }
            let imageData = this.extractImageFromDataUrl(value);
            let rawData : string = imageData != null ? imageData : value;
            let binaryString : string = "";
            try {
                binaryString = atob(rawData);
            } catch (e) {
                if (!this.validateStringAsBase64(rawData)) {
					throw "Supplied Data is not a valid base64-String Util.convertBase64ToBinaryString ";
				} else {
					throw "atob-Error in Util.convertBase64ToBinaryString ";
				}
            }
    ...
    private static convertBinaryStringToUint8Array(value : string) : Uint8Array {
    ...
    private static extractImageFromDataUrl(value : string) : string | null {
        let dataUrl : string = value;
        let dataUrlParts : string[] = dataUrl.split("base64,");
        let result : string | null = null;
        if (dataUrlParts.length === 2) {
            let extractedInfo : RegExpExecArray | null = /^data:(\w*\/\w*);*(charset=(?!charset=)[\w=-]*)*;*$/.exec(
                dataUrlParts[0]
            );
    ...
    private static validateStringAsBase64(value : string) : boolean {
        value = value || "";
        value.toString().trim();
        ...
        if (/^[A-Za-z0-9+/]+$/.test(value.substr(0, value.length - 2)) === false) {
            result = false;
        }

        if (/^[A-Za-z0-9/][A-Za-z0-9+/]|[A-Za-z0-9+/]=|==$/.test(value.substr(-2)) === false) {
            result = false;
        }
