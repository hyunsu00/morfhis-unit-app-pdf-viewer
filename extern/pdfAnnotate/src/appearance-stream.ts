...
export class ImageObj {
    ...
    new_object: boolean = false // indicates to the factory that this object must be created when writing the document
    type: string = "/Image"
    type_encoded: number[] = [47, 73, 109, 97, 103, 101] // = '/Image'

    stream_data: Uint8Array | undefined = undefined
    bitsPerComponent: number = 8
    colorSpace: string = 'DeviceGray'
    width: number = 0
    height: number = 0

    mask_object_id: ReferencePointer | undefined = undefined

    constructor(
        object_id: ReferencePointer | undefined = undefined, 
        stream_data: Uint8Array | undefined = undefined,
        bitsPerComponent: number | undefined = undefined,
        colorSpace: string,
        width: number,
        height: number,
        mask_object_id: ReferencePointer | undefined = undefined) {
        ...
        this.mask_object_id = mask_object_id
    }

    public writeImage(cryptoInterface : CryptoInterface) : number[] {
        if(!this.object_id)
            throw Error("object_id of XObject not set")
        ...
        ret = ret.concat(WriterUtil.SUBTYPE)
        ret.push(WriterUtil.SPACE)
        ret = ret.concat(Util.convertStringToAscii('/Image'))
        ret.push(WriterUtil.SPACE)

        // /BitsPerComponent 8
        ret = ret.concat(Util.convertStringToAscii('/BitsPerComponent'))
        ret.push(WriterUtil.SPACE)
        ret = ret.concat(Util.convertNumberToCharArray(this.bitsPerComponent))
        ret.push(WriterUtil.SPACE)

        // /ColorSpace
        ret = ret.concat(WriterUtil.COLORSPACE)
        ret.push(WriterUtil.SPACE)
        ret = ret.concat(Util.convertStringToAscii(`/${this.colorSpace}`))
        ret.push(WriterUtil.SPACE)

        // /HRes
        ret = ret.concat(Util.convertStringToAscii('/HRes'))
        ret.push(WriterUtil.SPACE)
        ret = ret.concat(Util.convertNumberToCharArray(96))
        ret.push(WriterUtil.SPACE)

        // /VRes
        ret = ret.concat(Util.convertStringToAscii('/VRes'))
        ret.push(WriterUtil.SPACE)
        ret = ret.concat(Util.convertNumberToCharArray(96))
        ret.push(WriterUtil.SPACE)

        // /Height
        ret = ret.concat(Util.convertStringToAscii('/Height'))
        ret.push(WriterUtil.SPACE)
        ret = ret.concat(Util.convertNumberToCharArray(this.height))
        ret.push(WriterUtil.SPACE)

        // /Width
        ret = ret.concat(Util.convertStringToAscii('/Width'))
        ret.push(WriterUtil.SPACE)
        ret = ret.concat(Util.convertNumberToCharArray(this.width))
        ret.push(WriterUtil.SPACE)

        // /Mask
        if (this.mask_object_id) {
            ret = ret.concat(Util.convertStringToAscii('/SMask'))
            ret.push(WriterUtil.SPACE)
            ret = ret.concat(WriterUtil.writeReferencePointer(this.mask_object_id, true))
            ret.push(WriterUtil.SPACE)
        }
        
        let stream_data: Uint8Array = (this.stream_data) ? this.stream_data : new Uint8Array()
        return WriterUtil.writeStreamObject(this.object_id, ret, new FlateStream(stream_data, undefined, true, cryptoInterface, this.object_id))
    }
}
